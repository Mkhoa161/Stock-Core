...
- [x] **Phase 8: Pagination** — Backend and frontend support paginated company list with URL-synced state and debounced search (completed 2026-05-21)
...
**Success Criteria** (what must be TRUE):
  ...
  3. EC2 user-data script ran successfully — Docker is installed and ECR authentication was attempted (verified via `/var/log/user-data.log` on the instance)
  3b. *(Phase 7)* A `curl` against the EC2 Elastic IP on port 3000 returns HTTP 200 after the first CI/CD deploy pushes an image to ECR
...
**Wave 1** *(foundation — blocks all other waves)*
...
Cross-cutting constraints: All resources in us-east-1; VPC ID and subnet IDs from 05-01 referenced by all subsequent plans; NAT Gateway required for Lambda VPC egress (~$32/month).
...
**Wave 1** *(single autonomous plan — three independent Docker files)*
- [ ] 06-01-PLAN.md — 2-stage non-root Dockerfile + cleaned dev compose (auth vars removed) + ECR-ready prod compose via IMAGE_URI (DOCK-01, DOCK-02, DOCK-03)
...
**Wave 1** *(backend foundation — blocks frontend)*
- [x] 08-01-PLAN.md — Backend pagination + server-side search: `getAllCompaniesWithLatestDataPaginated`, GET / returns `{ data, total, page, limit, totalPages }`, Jest unit tests (PAGI-01)

**Wave 2** *(blocked on Wave 1 — needs the paginated response shape)*
- [x] 08-02-PLAN.md — Frontend: `PaginatedCompaniesResponse` type, `useDebounce` hook, rewritten `useCompanies(page, search)`, URL-synced `CompaniesTable` with pagination controls + debounced search, Suspense boundary on companies page (PAGI-02, PAGI-03, PAGI-04, PAGI-05)
**UI hint**: yes
